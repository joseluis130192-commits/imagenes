...
export async function guardarImagen(b64, formato, ficha) {
  ...
  if (subidaError) throw new Error(`No se pudo subir la imagen: ${subidaError.message}`);
  ...
  if (insertError) throw new Error(`No se pudo guardar el registro: ${insertError.message}`);
